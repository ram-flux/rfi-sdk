...
impl Pin {
    pub fn new(
        ...
        let wpriv_key = wpriv.unwrap_or_else(|| vec![]);
        Self {
            passwd,
            salt,
            wpriv_key,
    ...
    pub fn derive_key(&self) -> Result<(Vec<u8>, Vec<u8>), Box<dyn std::error::Error>> {
        let mut dk = [0u8; 32];
        pbkdf2_hmac::<Sha256>(self.passwd.as_slice(), self.salt.as_slice(), 100_000, &mut dk);
    ...
    pub fn encrypt(
        ...
        let ciphertext = cipher
            .encrypt(nonce, self.wpriv_key.as_ref())
...
mod tests {
    ...
    #[test]
    fn is_check() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn it_work() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pin = Pin::new(b"123456".to_vec(), b"some_salt".to_vec(), Some(wallet_priv_key.clone()));
        let (aes_key, aes_iv) = pin.derive_key()?;
        ...
        let aes_iv_b64 = base64::encode(&aes_iv);
