...
impl DbConnection {
    pub async fn init_user_database(url: String) -> Result<(), crate::DatabaseError> {
        let mut conn = crate::db::DbConnection::default();
        conn.set_migrator(crate::db::Migrator::Pri);
    ...
    pub async fn init_pub_database(url: String) -> Result<(), crate::DatabaseError> {
        let mut conn = crate::db::DbConnection::default();
        conn.set_migrator(crate::db::Migrator::Pub);
        let migrator = conn.migrator();
        let db = conn.set_uri(Some(url)).init_database().await?;
    ...
    pub async fn run_migrator(&self) -> Result<(), crate::Error> {
        ...
        //             .into(),
        //     ))?
        self.migrator().run(&sqlite_pool).await.map_err(|_| {
            crate::Error::BadRequest(
                crate::InitDatabaseError::DatabaseError(crate::DatabaseError::MigrationRunFailed)
                    .into(),
            )
        })
    }
