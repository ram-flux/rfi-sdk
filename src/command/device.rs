...
pub async fn init_device(
    ...
    #[cfg(not(feature = "mock"))]
    {
        ...
        let user_id = worker.next_id().unwrap() as u32;
        let device_id = worker.next_id().unwrap() as u32;
        let account_id = worker.next_id().unwrap() as u32;
        ...
        crate::service::device::new_device(device, device_id).await;
        ...
        crate::service::account::new_account(account, account_id).await;
        Ok(device_id).into()
    }
}
...
pub async fn update_token(device_id: u32) -> crate::response::Response<String> {
    ...
    #[cfg(not(feature = "mock"))]
    {
        ...
        crate::service::device::update_token(token, device_id).await;
        Ok(new_token.to_string()).into()
    }
}
...
pub async fn del_device(device_id: u32) -> crate::response::Response<()> {
    ...
    #[cfg(not(feature = "mock"))]
    {
        crate::service::device::del_device(device_id).await;
        Ok(()).into()
    }
}
...
mod test {
    ...
    async fn test_init_device() {
        ...
        let res = crate::command::device::init_device(
    ...
    async fn test_update_token() {
        ...
        let res = crate::command::device::update_token(device_id).await;
        println!("res: {res:#?}");
    }
    ...
    async fn test_del_device() {
        ...
        let device_id = 3393327105;
        let res = crate::command::device::del_device(device_id).await;
        println!("res: {res:#?}");
    }
}
